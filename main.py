...
def dice_loss(input, target, smooth=1e-5):
    ...
    intersection = (input_argmax.float() * target_one_hot).sum(dim=(2, 3))
    union = input_argmax.float().sum(dim=(1, 2)) + target_one_hot.sum(dim=(2, 3))
...
def focal_loss(input, target, alpha=None, gamma=2):
    input_softmax = F.softmax(input, dim=1)
    pt = input_softmax.gather(1, target.unsqueeze(1))
    ...
    if alpha is None:
        ...
        # Ensure alpha tensor matches the number of classes in your problem
        assert alpha.size(0) == input.size(1), "Alpha tensor size does not match the number of classes"

        # Repeat alpha tensor to match the shape of focal_weight and ce_loss tensors
        alpha_expanded = alpha.unsqueeze(1).unsqueeze(1).repeat(1, focal_weight.size(1), focal_weight.size(2), focal_weight.size(3))

        focal_loss = alpha_expanded * focal_weight * ce_loss
...
class networkFunctions():
    ...
    def create_network_functions(self, network):
        ...
        def train_function(Xs, Ys, hist_boxes, hist_boxes_gt, loss_weights, network):
            ...
            focal_alpha1 = torch.cuda.FloatTensor(loss_weights[3])
            focal_alpha2 = torch.cuda.FloatTensor(loss_weights[2])
            focal_alpha3 = torch.cuda.FloatTensor(loss_weights[1])
            focal_alpha4 = torch.cuda.FloatTensor(loss_weights[0])
            dice_smooth = 1e-5
            loss = 0.0
            for idx, (out, yss) in enumerate(zip(outputs, Yss_argmax)):
              if idx != 0:
                patch_size_h = int((out.size(2) / 2 ** idx))
                patch_size_w = int((out.size(3) / 2 ** idx))
                pred_re = out.unfold(2, patch_size_h, patch_size_h).unfold(3, patch_size_w, patch_size_w).contiguous()
                gt_re = yss.unfold(1, patch_size_h, patch_size_h).unfold(2, patch_size_w, patch_size_w).contiguous()
                pred_re_merged = pred_re.view(pred_re.size(0), pred_re.size(1), -1, pred_re.size(-2), pred_re.size(-1))
                gt_re_merged = gt_re.view(gt_re.size(0), -1, gt_re.size(-2), gt_re.size(-1))
                grids_in_each_column = int(out.shape[2] / patch_size_h)
                grids_in_each_row = int(out.shape[3] / patch_size_w)
                num_grids = grids_in_each_column * grids_in_each_row
                assert (num_grids == pred_re_merged.size(2))
                assert (num_grids == gt_re_merged.size(1))
                max_loss = -float("inf")
                for ng in range(num_grids):
                  patch_out = pred_re_merged[:, :, ng]
                  patch_gt = gt_re_merged[:, ng]
                  patch_loss = dice_loss(patch_out, patch_gt, smooth=dice_smooth)
                  if patch_loss > max_loss:
                   max_loss = patch_loss
                loss_ = max_loss
              else:
               loss_ = focal_loss(out, yss, alpha=focal_alpha1)
              if idx != 0:
                    loss_ = self.gwta_loss(out, yss, dice_loss, grid_factor=np.power(2, idx))
              loss += loss_
...
def get_boxed_img(image, h_map, w_map, gt_pred_map, prediction_downscale, thickness=1, multi_colours=False):
    ...
    for y, x in zip(Y, X):
        ...
        if h//2 in BOXES[3] or h//2 in BOXES[2]:
            t = 1
            if gt_pred_map[y, x] > 0.8:
                selected_colour = (0, 0, 255)  # Color for overcrowded areas (red)
        else:
            t = thickness
